---
source: https://gitcode.com/dcloud/docs-common/edit/main/plugin/uts-plugin.md
---

<!-- GENERATED BY docs-sync
hash: 3b7ddebf04834a6c149a2f322825bd5096d1a431ec7c81bb839a423159159bc4
-->
...
│	│	├─Resources               //iOS原生依赖的资源文件存放目录，合并到应用的Main Bundle中，可选
│	│	├─EmbedResources          //iOS原生依赖的内嵌资源文件存放目录，合并到编译插件生成动态库的Framework Bundle中（HBuilderX5.08+支持），可选
...
│	├─web                         //web平台目录
│	│	├─package.json            //web平台插件依赖配置文件，使用js包管理工具（npm、pnpm、yarn等）安装依赖即可
│	│	└─index.uts
...
│	└─index.uts                   // 跨平台平台插件入口，可选
└─package.json                    // 插件清单文件，必需
</code>
</pre>


根目录 index.uts 文件是程序主入口。如果插件根目录下没有指定平台的目录下的 index.uts 文件，则会寻找根目录下的 index.uts 文件。
...
|Resources			|iOS平台插件需要合并到应用Main Bundle中的资源文件存放目录				|
|EmbedResources	|iOS平台插件需要合并到编译插件生成动态库Framework Bundle中的资源文件存放目录（HBuilderX5.08+支持）			|
...
iOS平台保存到动态库Framework Bundle中的原生资源目录，每个uts插件会编译成一个独立的动态库Framework。云端打包时会将此目录下的所有文件添加到动态库Framework Bundle中。
...
app-harmony文件夹存放uts插件编译到鸿蒙时的代码逻辑。
...
**注意事项**  
在 uvue 文件中，请勿直接导入uts插件内部 utssdk 目录下的 uts 文件。以下为错误示例：
```ts
//在uvue中导入uts文件是错误的
// #ifdef APP-ANDROID
import { myApi, myApiSync, MyApiOptions } from "@/uni_modules/uts-api/app-android/index.uts";
// #endif
// #ifdef APP-IOS
import { myApi, myApiSync, MyApiOptions } from "@/uni_modules/uts-api/app-ios/index.uts";
// #endif
// #ifdef APP-HARMONY
import { myApi, myApiSync, MyApiOptions } from "@/uni_modules/uts-api/app-harmony/index.uts";
// #endif
```

> 在 JS 引擎下直接引入 uts 文件可能会被错误编译为 JavaScript，进而导致编译异常。  
