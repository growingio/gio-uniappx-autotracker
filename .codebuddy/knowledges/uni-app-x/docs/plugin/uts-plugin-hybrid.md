---
source: https://gitcode.com/dcloud/docs-common/edit/main/plugin/uts-plugin-hybrid.md
---

<!-- GENERATED BY docs-sync
hash: 7769aaa78adf3fc893ed37331b3ae34cacb15b983bcf0aab805d9e44fce78bab
-->
...
开发者只需要把 `Kotlin`/`swift`/`java`/`ArkTS`/`Js` 代码放在`UTS插件`目录中，就可以通过 `UTS`直接使用这些代码。
...
web和小程序平台uts会编译为js代码，在各个平台下可以直接使用js包管理工具安装依赖并使用。以下面的目录结构为例，`web/package.json`文件内可以配置web平台可用的依赖，可直接使用常见的js包管理工具（npm、pnpm、yarn等）安装。
