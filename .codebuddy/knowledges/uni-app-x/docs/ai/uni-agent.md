...
hash: 39891147db7be201478d6d60a321e8b4c6a971271798fcafd3b28ce4040b3590
-->
...
默认使用的是全球顶尖的 AI Coding 模型。也可以在uni-agent的设置中选择deepseek v4。

#### 订阅套餐说明

订阅套餐适合希望持续使用 uni-agent 的用户。各套餐按月订阅，订阅后可在 uni-agent 中直接使用 DCloud 账户自动下发的模型配置与额度。
...
如果不需要订阅套餐，也可以直接购买补充资源包来使用 uni-agent 功能。资源包没有 5 小时限额和 7 天周限额的限制，适合临时补充额度或低频按需使用。

| 资源包 | 价格 | 约等于 Token 数 | 有效期 |
| --- | --- | --- | --- |
| 1000 万积分 | ¥9.90 | 约 20M token | 30 天 |
| 5000 万积分 | ¥39 | 约 100M token | 60 天 |
| 1.5 亿积分 | ¥99 | 约 300M token | 90 天 |
...
设置窗口顶部提供了 **模型选择** 功能，用于控制 uni-agent 当前优先使用哪一类模型来源。

- **默认模型**：使用 DCloud订阅的默认模型。这是推荐选项，uni-agent 内置的技能（Skills）、知识库与调度策略主要基于该模型做过完整验证。
- **聚合中转**：使用 DCloud订阅，但会切换到聚合中转线路。适合在默认模型暂时不可用，或需要对比不同模型时使用。
...
> 注意：
> - `聚合中转` 属于第三方中转线路，可能存在数据安全与稳定性风险。
> - `自定义Key` 或其它自定义模型来源，未必完整适配 uni-agent 的技能、知识库与项目分析链路，实际效果可能弱于默认模型。
