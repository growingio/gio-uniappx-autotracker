...
* 调整 `db_init.json` 按collection拆分，每个collection由schema.json、index.json、init_data.json三个文件描述 [详情](https://doc.dcloud.net.cn/uniCloud/hellodb.html#init-db)
...
#### 2023-04-11
* 【重要】新增 uni-cloud-ai 扩展库，提供AI对话能力 [详情](https://doc.dcloud.net.cn/uniCloud/uni-ai.html)
* 新增 JQL语法 触发器内副表读取事件增加 primaryCollection 参数，值为本次联表查询的主表表名
* 调整 JQL语法 schema 内配置的动态 enum 校验数据时移除仅枚举500条的限制

#### 2023-03-31
* 新增 jql语法支持查询某字段和另一个表指定字段相等的记录 [详情](https://doc.dcloud.net.cn/uniCloud/jql.html#enhanced-in)
...
#### 2022-12-16
* JQL语法 新增 触发器内可以获取用户信息和本次数据库操作结果 [用户信息](https://doc.dcloud.net.cn/uniCloud/jql-schema-ext.html#user-info)、[执行结果](https://doc.dcloud.net.cn/uniCloud/jql-schema-ext.html#result)
...
#### 2022-09-05
  + 新增 阿里云 支持申请云存储独立CDN域名，避免因其他服务空间违规导致云存储被封禁 [详情](https://doc.dcloud.net.cn/uniCloud/storage.html#aliyun-cdn-domain)
