...
## 配套uniCloud成本
但在实际使用中需要依赖`uniCloud`云服务。如使用uniCloud支付宝云服务空间，每条大约需要多花0.0000139元，几乎可以忽略不计。您也可以粗略认为每条短信的费用为0.0360139元/条。费用计算规则如下：
...
结论：如果你的`短信`业务平均每天发送条数为10000条，使用支付宝云服务空间后，对应云函数每天大概消耗0.139元，对比之前的短信费用，平均每次调用多花0.0000139元，几乎可忽略不计。
