...
// 如果拷贝此内容切记去除注释
{
  ...
  "service": {
    ...
    "storage": {
      "mode": "ext", // 可选值：默认不填（使用内置云存储），"ext"（使用扩展存储）
      "provider": "qiniu", // 扩展存储服务商，mode为ext时必填
      "domain": "", // CDN域名，mode为ext时必填
      "bucketName": "", // 存储桶名称，选填，缺省时自动获取
      "bucketSecret": "" // 存储桶密钥，选填，缺省时自动获取
    }
  }
}
```
...
### 云存储配置@storage-config <Badge text="uni-id-pages 1.1.28+" />

> 此功能需要 uni-id-pages >= 1.1.28 版本

uni-id-co 在三方登录拉取头像、实名认证落库照片等场景会将文件上传到云存储。默认使用内置云存储，可通过 `service.storage` 配置使用[扩展存储](https://doc.dcloud.net.cn/uniCloud/ext-storage/dev.html)（如七牛云等第三方存储服务）。
...
|字段								|类型		|必填	|说明																								|
|--									|--			|--		|--																									|
|mode								|String	|否		|`"ext"` 使用扩展存储；不填或缺省则使用内置云存储								|
|provider						|String	|是		|扩展存储服务商，如 `"qiniu"`；mode为ext时必填									|
|domain							|String	|是		|CDN域名；mode为ext时必填																		|
|bucketName					|String	|否		|存储桶名称，缺省时自动获取																	|
|bucketSecret				|String	|否		|存储桶密钥，缺省时自动获取																	|

**使用前提**

需要在云函数 `uni-id-co` 中关联 `uni-cloud-ext-storage` 扩展库：右键 `uni-id-co` → 管理公共模块或扩展库依赖 → 勾选 `uni-cloud-ext-storage`。如未关联，云函数运行时会提示错误。
...
- [uni-id-co中文语言包](https://gitcode.com/dcloud/hello_uni-id-pages/blob/master/uni_modules/uni-id-pages/uniCloud/cloudfunctions/uni-id-co/lang/zh-hans.js)
- [uni-id-common中文语言包](https://gitcode.com/dcloud/uni-id-common/blob/master/src/lang/zh-Hans.js)
...
>
> [参考从文件中导入数据](https://doc.dcloud.net.cn/uniCloud/hellodb.html#import)
...
[从文件中导入数据说明](https://doc.dcloud.net.cn/uniCloud/hellodb.html#import)
