...
|uniIdCo.getTempFileURL()		|获取文件临时访问链接 [详情](#get-temp-file-url)				|
|uniIdCo.getUploadFileOptions()|获取上传参数 [详情](#get-upload-file-options)				|
...
#### 获取文件临时访问链接@get-temp-file-url <Badge text="uni-id-pages 1.1.28+" />

> 此接口需要 uni-id-pages >= 1.1.28 版本

获取文件的临时访问链接。用于前端展示扩展存储中的文件（如头像）。

当文件存储在内置云存储时，前端可直接使用 `uniCloud.getTempFileURL` 换链；当使用扩展存储时，必须通过此接口在云端换取临时链接。

**注意**：此接口要求用户已登录（在 `permission.js` 中配置了 `auth: true`）。

**接口形式**

```js
const uniIdCo = uniCloud.importObject('uni-id-co')
const res = await uniIdCo.getTempFileURL({
  fileID: 'fileID'
})
console.log(res.tempFileURL) // 临时访问链接
```
...
|字段		|类型		|必填	|说明													|
|--			|--			|--		|--														|
|fileID	|String	|是		|文件 fileID（cloud://、扩展存储 URI 或 https://链接）|
...
|tempFileURL|String	|临时访问链接		|

#### 获取上传参数@get-upload-file-options <Badge text="uni-id-pages 1.1.28+" />

> 此接口需要 uni-id-pages >= 1.1.28 版本

获取上传文件所需的参数。返回的 `mode` 字段用于前端判断走内置存储还是扩展存储：
- `mode === 'builtin'`：前端走 `uniCloud.uploadFile({ filePath, cloudPath })`
- `mode === 'ext'`：前端走 `uni.uploadFile({ ...uploadFileOptions, filePath })`
...
```js
const uniIdCo = uniCloud.importObject('uni-id-co')
const options = await uniIdCo.getUploadFileOptions({
  cloudPath: 'user/avatar/xxx.jpg'
})
...
|--								|--			|--																			|
|errCode						|Number	|错误码，0表示成功													|
|mode							|String	|`"builtin"` 内置存储，`"ext"` 扩展存储						|
|cloudPath					|String	|云端文件路径（使用扩展存储时可能与入参不同）	|
|fileID						|String	|文件ID，mode为ext时返回											|
|uploadFileOptions	|Object	|uni.uploadFile 所需参数（url、name、formData），mode为ext时返回	|
