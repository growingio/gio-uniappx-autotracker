...
// 如果拷贝此内容切记去除注释
{
  ...
  "preferedAppPlatform": "app", // 新增于uni-id 3.3.12，指定app端对应的PLATFORM名称，用于处理app-plus和app的兼容问题，详细说明见：https://doc.dcloud.net.cn/uniCloud/uni-id?id=prefered-app-platform
  "preferedWebPlatform": "web", // 新增于uni-id 3.3.22，指定web端对应的PLATFORM名称，用于处理web和h5的兼容问题，详细说明见：https://doc.dcloud.net.cn/uniCloud/uni-id?id=prefered-web-platform
...
注意：**在uniCloud admin中，封装了可视化的用户、权限、角色的管理，新增删除修改均支持。**无需自己维护。[详见](https://doc.dcloud.net.cn/uniCloud/admin?id=mutiladmin)
...
```js
{
  ...
  version: 1 // 密钥版本，关于此字段请参考https://doc.dcloud.net.cn/uniCloud/uni-id?id=modifysecret
}
```
...
exports.main = async function(event, context) {
  const {
    appid,
    platform
  } = event.queryStringParameters // 不同类型的请求获取参数的方式略有差异，具体如何取参数请参考：https://doc.dcloud.net.cn/uniCloud/http
  context.APPID = appid
