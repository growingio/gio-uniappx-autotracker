...
#### 2021-09-16
+ 阿里云 调整 跨域配置对前端网页托管自定义域名生效 [详情](https://doc.dcloud.net.cn/uniCloud/quickstart?id=useinh5)

#### 2021-09-01
+ 【重要】clientDB 新增 支持使用 getTemp 对主表、副表过滤后再联表查询，大幅提升联表查询性能 [详情](https://doc.dcloud.net.cn/uniCloud/clientdb?id=lookup-with-temp)
+ 阿里云 新增 redis扩展能力，加速云函数响应 [详情](https://doc.dcloud.net.cn/uniCloud/redis)
...
+ uni-id 调整 3.1.1版本发布，使用兼容uniCloud响应体规范的新错误码格式 [详情](https://doc.dcloud.net.cn/uniCloud/uni-id?id=errcode)
...
#### 2021-05-25
+ web控制台 腾讯云 调整 云函数Url化需要配置CNAME为腾讯云给定的域名，此前配置为默认域名 [详情](https://doc.dcloud.net.cn/uniCloud/http)
...
#### 2020-12-10
+ web控制台 新增 阿里云新增稀疏索引 [详情](https://doc.dcloud.net.cn/uniCloud/hellodb?id=dbindex)
...
+ uni-id 调整 2.0.0版本起encryptPwd接口返回值调整 [详情](https://doc.dcloud.net.cn/uniCloud/uni-id?id=encrypt-password)
+ uni-id 新增 修改passwordSecret功能 [详情](https://doc.dcloud.net.cn/uniCloud/uni-id?id=modifysecret)

#### 2020-11-13
+  阿里云支持事务（startTransaction方式，暂不支持runTransaction）[详情](https://doc.dcloud.net.cn/uniCloud/cf-database?id=start-transaction)
...
#### 2020-10-24
+ 【重要】新增 clientDB 支持 `jql` 查询语法，大幅降低数据库操作难度 [详情](https://doc.dcloud.net.cn/uniCloud/database?id=jsquery)、大幅简化联表查询 [详情](https://doc.dcloud.net.cn/uniCloud/database?id=lookup)
+ 【重要】新增 uni-clientDB 组件，在前端通过组件直接获得云数据库内容，并直接绑定到界面上，大幅提升开发效率 [详情](https://doc.dcloud.net.cn/uniCloud/uni-clientdb-component)
+ 【重要】调整 clientDB内置，云端不再需要单独的clientDB云函数，前端无需引用clientDB的js sdk，直接在前端写`const db = uniCloud.database()`即可 [详情](https://doc.dcloud.net.cn/uniCloud/database)
+ 【重要】调整 uni-clientDB-actions 目录调整到 cloudfunctions 根目录 [详情](https://doc.dcloud.net.cn/uniCloud/database?id=action)
...
#### 2020-10-13
+ 腾讯云 全面开放企业用户按量计费服务空间的购买 [详情](https://doc.dcloud.net.cn/uniCloud/price?id=price-info)

#### 2020-09-29
+ 腾讯云 开放包年包月套餐购买 [详情](https://doc.dcloud.net.cn/uniCloud/price?id=price-month)

#### 2020-09-26
+ 【重要】新增 腾讯云 云函数固定出口IP，可用于微信公众号开发等要求配置ip的场景 [详情](https://doc.dcloud.net.cn/uniCloud/cf-functions?id=eip)
+ 【重要】uni-clientDB 2.0 重大更新，可完整方便的控制权限和数据验证。大多数场景不再需要编写云函数 [详情](https://doc.dcloud.net.cn/uniCloud/clientdb)
...
#### 2020-09-16
+ 腾讯云 支持云函数固定出口IP，支持微信公众号开发 [详情](https://doc.dcloud.net.cn/uniCloud/cf-functions?id=eip)
...
#### 2020-08-04
+ 新增 3个内置短信模板 [详情](https://doc.dcloud.net.cn/uniCloud/send-sms)
...
#### 2020-07-21
+ 【重要】新增 本地运行云函数，可连接远程数据库和云存储，大幅提升开发效率，同时方便数据导入导出 [详情](https://doc.dcloud.net.cn/uniCloud/local)
