...
如果要控制云存储的权限可以使用自定义登录，[uniCloud腾讯云自定义登录](https://doc.dcloud.net.cn/uniCloud/authentication)

#### 对接微信支付

uniCloud提供了uniPay来实现支付功能，同时支持微信和支付宝[unipay文档](https://doc.dcloud.net.cn/uniCloud/unipay)
...
- 数据校验能力，参考：[DB schema](https://doc.dcloud.net.cn/uniCloud/schema)
- 在数据库操作前/后执行额外操作，参考：[action](https://doc.dcloud.net.cn/uniCloud/clientdb?id=action)
