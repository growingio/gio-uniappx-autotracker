...
- 云端基于 [uniCloud](https://doc.dcloud.net.cn/uniCloud/) 实现。后台管理是 [uni-admin](https://doc.dcloud.net.cn/uniCloud/admin.html) 框架的插件。

- 数据库遵循 [opendb](https://doc.dcloud.net.cn/uniCloud/opendb.html) 规范
...
> 运行 uni-admin 到浏览器，在左侧菜单 `系统管理 -> 应用管理` 中新增应用后，即可在 `App升级中心` 发布该应用的版本 [详情](https://doc.dcloud.net.cn/uniCloud/admin.html#app-manager)
...
以阿里云[按量计费](https://doc.dcloud.net.cn/uniCloud/price.html#aliyun-postpay)为例，预测下`uni-upgrade-center`在不同用户规模下的资源消耗及对应费用，帮助大家明智选择。
...
我们按照uniCloud官网列出的[按量计费](https://doc.dcloud.net.cn/uniCloud/price.html#aliyun-postpay)规则，可以得出如下云函数资源消耗计算公式：
...
按照[uniCloud官网](https://doc.dcloud.net.cn/uniCloud/price.html#aliyun-postpay)介绍，云存储费用 = `容量费 + 下载操作次数计费点 + 上传操作次数计费点 + CDN流量费`。
...
#### 在测试时，如果使用标准基座运行，则无法检测出 App 更新

原因是因为标准基座获取到的 appId 是固定的，而不是 manifest 中配置的。建议使用自定义基座测试

#### 使用支付宝云无法查询到 App 更新

- 更新 uni-admin 到 `2.5.4+`
- 尝试在`database/opendb-app-versions.index.json`文件右键“初始化云数据库索引”后重试。
- 如果`database/opendb-app-versions.index.json`不存在，请在uniCloud控制台-服务空间-云数据库找到`opendb-app-versions`表，为`platform`字段创建数据索引后重试。

#### HarmonyOS 运行期间 wgt 升级失败

HarmonyOS 上由于和运行期间调试热刷冲突，现在只能通过 `App-Harmony-本地打包 -> 生成安装包` 然后将 `unpackage/dist/build/app-harmony` 使用 DevEco 打开来测试 wgt 更新
