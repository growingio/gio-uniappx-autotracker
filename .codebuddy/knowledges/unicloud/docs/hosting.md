...
`前端网页托管`，自带一个测试域名，仅用于产品体验及测试可快速体验前端网页部署的完整流程，该域名有如下限制：
  ...
  + 腾讯云限速100K/s。自**2025年10月9日**起新建的腾讯云服务空间，默认域名将对通过浏览器直接访问的请求（Sec-Fetch-Mode 请求头的值为 navigate）校验有效期，对其他请求添加 Content-Disposition: attachment 响应头（浏览器直接打开会提示下载）。对于已失效的默认域名，需用户在控制台点击续期后恢复使用。
...
如果阿里云服务空间已绑定域名，在未主动操作解绑的情况下，通过阿里云官网申诉找回域名并绑定在了阿里云官网的CDN产品，此时服务空间上的域名便无法正常使用，也无法解绑，会有如下提示：`InvalidDomain.NotFound: The domain does not belong to you.`此时需用户提供阿里云官网CDN绑定域名相关配置截图联系我们人工处理。

**注意：**
- 阿里云服务空间只能绑定一个自定义域名，域名需在阿里云接入备案。
- 腾讯云及支付宝云服务空间可绑定至多4个自定义域名，支付宝云同样需要域名在阿里云接入备案。
...
- 阿里云账号下所有服务空间限制每小时最多刷新10次
- 支付宝云文件夹限制每天只允许刷新1次，文件限制每天允许刷新20次
- 缓存刷新后均约10-30分钟生效，各地区CDN节点生效时间可能不一
...
服务空间已默认支持`localhost`及`前端网页托管默认域名`跨域请求云函数，如果需要使用内网地址或者前端网页托管绑定的自定义域名访问云函数，需单独添加跨域设置。内网地址需填写带端口的完整地址，如`192.168.10.11:5173`。
...
阿里云前端网页托管在绑定自定义域名后，支持配置CDN安全策略：[https://doc.dcloud.net.cn/uniCloud/cdn-security-policy.html](storage/cdn-security-policy.md)
