...
可通过 `uni-id-co` 获取，[详情](https://doc.dcloud.net.cn/uniCloud/uni-id/summary.html#save-user-token)
...
// uniCloud/cloudfunctions/common/uni-config-center/uni-open-bridge/config.json
{
  "schedule": {
    "__UNI__xxxxxx": { // dcloudAppid, 需要和 `uni-config-center` uni-id中的配置一致
      ...
      "weixin-mp": { // 平台，目前仅支持 微信小程序、微信 H5，详情参见 https://doc.dcloud.net.cn/uniCloud/uni-open-bridge#platform
...
在数据库`opendb-open-data`中会看到数据。需要在[uniCloud web控制台](https://unicloud.dcloud.net.cn/)--》云数据库中或在项目的uniCloud--》database目录下，**手动创建数据表`opendb-open-data`**。

如开通redis则在redis的`uni-id`分组中查看（推荐开通redis以获取更好的性能）。
