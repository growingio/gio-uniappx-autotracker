...
> 欢迎加入[uniCloud官方技术交流群](https://im.dcloud.net.cn/#/?joinGroup=6581841e3e585b555acec05c)
...
2. uniCloud基于serverless，开发者无需了解linux、nginx，无需熟悉负载均衡、高并发处理，不用关心系统补丁和DDoS攻击，只需要用js写好云端业务代码，上传到uniCloud即可。
...
如果schema2code仍不能满足你的需求，需要单独开发，那么只要操作数据库，[clientDB](https://doc.dcloud.net.cn/uniCloud/clientdb)就是推荐使用的利器。
...
- 在传统mysql或MongoDB中，联表查询的写法非常麻烦，tree查询mysql根本没有，需要oracle等专业数据库。但在uniCloud中，只需在数据库的schema中定义两个表的字段之间的关系，比如是引用关系（foreignKey）、还是父子关系(parentKey)，前端就可以直接查询，轻松完成[联表查询](https://doc.dcloud.net.cn/uniCloud/clientdb?id=lookup)和[tree查询](https://doc.dcloud.net.cn/uniCloud/clientdb?id=gettree)
