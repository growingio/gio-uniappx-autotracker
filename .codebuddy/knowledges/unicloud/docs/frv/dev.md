...
- `uni-app 项目`: 调用[uni.getFacialRecognitionMetaInfo](https://uniapp.dcloud.net.cn/api/plugins/facialRecognitionVerify.html)
- `uni-app x 项目`: 调用[uni.getFacialRecognitionMetaInfo](https://doc.dcloud.net.cn/uni-app-x/api/facial-recognition-meta-info.html#getfacialrecognitionmetainfo)
...
- `uni-app 项目`: 调用[uni.startFacialRecognitionVerify](https://uniapp.dcloud.net.cn/api/plugins/facialRecognitionVerify.html)打开认证界面，通过刷脸操作获取认证结果。
- `uni-app x 项目`: 调用[uni.startFacialRecognitionVerify](https://doc.dcloud.net.cn/uni-app-x/api/facial-recognition-verify.html#startfacialrecognitionverify)打开认证界面，通过刷脸操作获取认证结果。
...
- 认证触发页面：负责向 uniCloud（云函数/云对象）发起请求，获取 certifyUrl。一旦收到 certifyUrl，页面将跳转到该认证地址，启动实人认证流程。
- 结果显示页面：用户在认证页面完成操作后，会自动跳转到此页面。该页面将接收 certifyId 参数，并使用它再次向 uniCloud 端请求最终的认证结果。

#### 接入流程
在网页端和uniCloud云函数中调用如下api，实现下图流程：
- 获取metaInfo：[uni.getFacialRecognitionMetaInfo](#window-get-meta-info)
- 云函数获取实人认证实例：[uniCloud.getFacialRecognitionVerifyManager()](#get-frv-manager)
- 云函数提交姓名、身份证号、metaInfo、结果页面url(业务定义)获取认证地址(certifyUrl)：[frvManager.getCertifyUrl()](#get-certify-url)
- 云函数使用certifyId获取认证结果：[frvManager.getAuthResult()](#get-auth-result)
...
sequenceDiagram
  ...
  activate user
  user->>+user: 获取设备信息metaInfo
  user->>+cf: 提交姓名、身份证号、metaInfo获取certifyUrl
  cf->>+service: 提交姓名、身份证号、metaInfo、结果页面url(业务定义)获取certifyUrl
  service-->>-cf: 返回certifyUrl
  cf-->>-user: 返回certifyUrl
  user->>+user: 跳转认证地址开始认证
  user->>+service: 认证页面自动提交人脸数据
  service-->>-user: 认证完成跳转结果页携带：认证ID和结果
  user->>+cf: 请求校验认证结果携带：certifyId
  cf->>+service: 请求认证结果
  service-->>-cf: 返回认证结果
  cf-->>-user: 返回最终实人认证结果
  deactivate user
...
#### 认证触发页面@window-get-meta-info

1.先获取设备信息，调用[uni.getFacialRecognitionMetaInfo](https://uniapp.dcloud.net.cn/api/plugins/facialRecognitionVerify.html)
...
|字段名		|类型		| 描述		|
|:-:			|:-:		|:-:		|
|metaInfo|String	|云函数获取认证服务的certifyUrl时，在入参metaInfo中传入该值	|
...
```js
  ...
  const todo = uniCloud.importObject('uni-face-recognition-co', {
        customUI: true
      })
      const metaInfo = uni.getFacialRecognitionMetaInfo()
      todo.getCertifyUrl({
...
export default {
  ...
  onLoad(e) {
    ...
    if (!response) {
        console.log('未获取到认证结果参数')
      return
    }
    try {
      this.response = JSON.parse(response)
    } catch (err) {
      console.log('认证结果参数解析失败')
      return
    }
    if (!this.response || !this.response.extInfo?.certifyId) {
     console.log('认证ID缺失')
      return
    }
    this.getAuthResult()
  ...
  methods: {
    getAuthResult() {
      const certifyId = this.response.extInfo.certifyId
      const todo = uniCloud.importObject('uni-face-recognition-co', { customUI: true })
      todo.getAuthResult({ certifyId }).then(res => {
...
#### 获取certifyUrl@get-certify-url

**接口形式**

```js
frvManager.getCertifyUrl(Object GetCertifyIdOptions)
```
...
|idCard			|String	|是		|-			|用户身份证号									|
|metaInfo		|String	|是		|-			|客户端获取设备信息返回的metaInfo	|			|
|returnUrl      |String |是		|-	       |业务定义 认证结果的回调通知地址，回调请求方式默认为GET，回调地址必须以https开头					|

**返回值**

|字段名		|类型		|必备	|说明																								|
|:-:			|:-:		|:-:	|:-:																								|
|certifyUrl|String	|是		|认证完成跳转结果页|
...
exports.main = async (event, context) => {
  ...
  const result = await frvManager.getCertifyUrl({
    realName: '张三',
    idCard: 'xxxxxx',
    metaInfo: '{"xx": "xx"}'，
...
`uni-frv-external-co`提供的接口，默认开启了鉴权验证。
