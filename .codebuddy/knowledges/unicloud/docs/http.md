...
- 自**2025年10月9日**起新建的腾讯云服务空间，默认域名将对通过浏览器直接访问的请求（Sec-Fetch-Mode 请求头的值为 navigate）校验有效期，对其他请求添加 Content-Disposition: attachment 响应头（浏览器直接打开会提示下载）。对于已失效的默认域名，需用户在控制台点击续期后恢复使用。
...
2. 单击【域名绑定】，可查看默认域名及自定义域名列表。

<img style="max-width:800px;height:auto;" src="https://web-ext-storage.dcloud.net.cn/unicloud/docs/SCR-20251021-oqmk.png"></img>

3. 点击【添加域名】，在弹出的窗口中进行配置

<img style="max-width:800px;height:auto;" src="https://web-ext-storage.dcloud.net.cn/unicloud/docs/SCR-20251021-oqog.png"></img>

其中`CNAME`的值腾讯云需要绑定成功后才能获取到，获取到之后将绑定域名的CNAME解析配置为该记录值。阿里云及支付宝云绑定前便可以获取到`CNAME`，需先配置解析然后操作绑定。

**注意**

- 腾讯云、阿里云每个服务空间最多绑定1个自定义域名，支付宝最多支持绑定3个自定义域名
...
- 阿里云、支付宝云绑定自定义域名要求此域名在阿里云接入备案信息，参考：[新增备案信息](https://help.aliyun.com/document_detail/36926.html)
- 腾讯云绑定自定义域名要求此域名在腾讯云接入备案信息，参考：[新增备案信息](https://cloud.tencent.com/document/product/243/97669)
- 腾讯云绑定的自定义域名同样对前端网页托管生效，即云函数绑定域名后可以直接访问前端网页托管
...
支付宝云不支持`-----BEGIN EC PRIVATE KEY----- xxx -----END EC PRIVATE KEY-----`这种EC算法格式的私钥，仅支持以下两种格式：
...
#### 阿里云自定义域名备用抗DDoS CNAME@ddos-cname

阿里云callFunction及url化默认域名的网关均具备抗DDoS攻击功能。但阿里云绑定自定义域名后分配的`fc-${SpaceID}-custom.next.bspapp.com`含有动态服务空间ID的CNAME不具备抗DDoS攻击功能。

因此阿里云提供了备用的CNAME，地址为：`ddos-http-custom.next.bspapp.com`。该CNAME适用于所有阿里云服务空间，如果域名解析到该地址，则url化地址具备抗DDoS攻击功能，但该CNAME仅支持5080（http协议）、50443（https协议）两个端口。

举例说明，假如服务空间云函数绑定了`test.xxx.com`域名，并将该域名的CNAME解析配置到`ddos-http-custom.next.bspapp.com`，则访问该自定义域名url化接口地址时需指定端口。比如空间某个函数的url化触发path设置为`request-test`，访问时需携带端口，如：`http://test.xxx.com:5080/request-test`或`https://test.xxx.com:50443/request-test`，上述两个地址分别对应http及https访问的路径。
...
```json
{
    "mpserverlessComposedResponse": true, // 返回集成响应是需要此字段为true
