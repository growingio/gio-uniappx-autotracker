...
简单查询条件包括以下几种，对应着db.command下的各种[操作符](https://doc.dcloud.net.cn/uniCloud/cf-database?id=dbcmd)以及不使用操作符的查询如`where({a:1})`。
...
详见：[https://doc.dcloud.net.cn/uniCloud/unicloud-db?id=page](https://doc.dcloud.net.cn/uniCloud/unicloud-db?id=page)
...
自`HBuilderX 3.1.0`起，JQL支持在云端数据库对字段进行一定的操作运算之后再返回，详细可用的方法列表请参考：[数据库常用运算方法](jql-operator-example.md)
...
如果使用uniCloud-db组件，则在组件的属性上增加一个 getone。[详见](https://doc.dcloud.net.cn/uniCloud/unicloud-db?id=props)
...
如果使用uniCloud-db组件，则在组件的属性上增加一个 getcount。[详见](https://doc.dcloud.net.cn/uniCloud/unicloud-db?id=props)
...
<a target="_blank" href="https://web-ext-storage.dcloud.net.cn/doc/unicloud/uni-map/uni-map.mp4">
...
JQL API 支持事务操作，与 `uniCloud.database()` 的[事务操作](cf-database.md#transaction)类似，事务操作可以将多条增删改操作合并成一个原子操作，要么全部成功，要么全部失败。
...
1. JQL事务不支持`doc.set`、`where.updateAndReturn`方法，如需使用请通过`uniCloud.database()`的事务操作。
2. JQL事务中`update`不支持使用更新操作符，如需使用请通过`uniCloud.database()`的事务操作。
3. JQL触发器中获取的`transaction`对象为`uniCloud.database()`的事务对象，可以用此参数判断是否在事务中执行。
...
**下面示例中使用了注释，实际使用时schema是一个标准的json文件不可使用注释。**完整属性参考[schema字段](https://doc.dcloud.net.cn/uniCloud/schema?id=segment)
...
// state参数格式如下
{
  ...
  auth: {
    ...
    permission // 通过uni-id获取的用户权限，需要使用1.1.9以上版本的uni-id，如果未获取或者获取失败permission值为[]，注意登录时传入needPermission才可以获取permission，请参考 https://doc.dcloud.net.cn/uniCloud/uni-id?id=rbac
  },
