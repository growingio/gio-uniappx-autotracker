...

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, null, null, null, null, null, null\); actions\.onSimpleTrack\(\$event\)"/)
}

{
  const code = '<template><view @click="onSimpleTrack"></view></template><script setup lang="uts">function onSimpleTrack() : void {}</script>'
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, null, null, null, null, null, null\); onSimpleTrack\(\)"/)
}

{
  ...
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, null, null, null, null, null, null\); importedHandler\(\$event\)"/)
}
...

{
  ...
  const output = transform(code)
  assert.match(output, /@longpress="_gioAutoTrackDispatch\(\$event, 0, null, 'press', null, 'longpress 区域', null, null, null\); onLongPress\(\)"/)
  assert.match(output, /@longtap="_gioAutoTrackDispatch\(\$event, 1, null, 'tap', null, 'longtap 区域', null, null, null\); onLongTap\(\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, null, null, null, null, null, null\); onPayload\(\{ \$event \}\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, null, null, null, null, null, null\); \(\(payload\) => onPayload\(payload\)\)\(\$event\)"/)
}

{
  ...
  assert.match(output, /<input[\s\S]*id="auto_blur_input"[\s\S]*@blur="_gioAutoTrackDispatch\(\$event, \d+, \$event, 'auto_blur_input', '42', 'blur 输入框', '[^']+', 'true', null\); onInputBlur\(\$event\)"/)
...

{
  ...
  assert.match(output, /@change="_gioAutoTrackDispatch\(\$event, 0, \$event, null, null, null, null, 'true', null\); onSliderChange\(\$event\)"/)
  assertSetupDispatcher(output)
}
...

{
  ...
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'doc_link', null, '文档', 'https:\/\/doc\.dcloud\.net\.cn', null, null\)"/)
...

{
  ...
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'dynamic_link', null, null, \(docUrl\), null, null\)"/)
}

{
  ...
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'custom_link', null, null, 'https:\/\/example\.com\/custom', null, null\)"/)
}

{
  ...
  const output = transform(code)
  assert.doesNotMatch(output, /data-src="https:\/\/doc\.dcloud\.net\.cn"/)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'dynamic_custom_link', null, null, \(customUrl\), null, null\)"/)
}

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'explicit_link', null, null, 'https:\/\/doc\.dcloud\.net\.cn', null, null\); onLinkClick\(\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'explicit_custom_link', null, null, 'https:\/\/example\.com\/custom', null, null\); onLinkClick\(\)"/)
  assert.doesNotMatch(output, /data-src="https:\/\/doc\.dcloud\.net\.cn"/)
}

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'explicit_dynamic_custom_link', null, null, \(customUrl\), null, null\); onLinkClick\(\)"/)
  assert.doesNotMatch(output, /data-src="https:\/\/doc\.dcloud\.net\.cn"/)
}
...

{
  ...
  const output = transform(code)
  assert.match(output, /gioHandleAutoChange\(\$event, 'onChange', null, null, null, null, null, null, null, \$event\)/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@blur="_gioAutoTrackDispatch\(\$event, 0, \$event, null, null, null, null, null, null\); onPasswordBlur\(\$event\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /v-on:tap\.stop="_gioAutoTrackDispatch\(\$event, 0, null, 'ast_event', '7', 'AST 事件', null, null, null\); onEventAndLabel\(\$event, '\$event'\)"/)
...

{
  ...
  const output = transformForPlatform(code, 'app-android')
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, \(item\.id\), \(item\.index\), \(item\.title\), \(item\.src\), \(item\.track\), \(item\.ignore\)\); onItemClick\(item\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /_gioAutoTrackDispatch\(\$event, 0, null, null, null, \(item\.title \?\? &quot;fallback&quot;\), null, null, null\)/)
}

{
  const code = `<template><input :type="'password'" @change="onDynamicLiteralType" /></template><script setup lang="uts"></script>`
  const output = transform(code)
  assert.match(output, /@change="_gioAutoTrackDispatch\(\$event, 0, \$event, null, null, null, null, null, null\); onDynamicLiteralType\(\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@click="_gioAutoTrackDispatch\(\$event, 0, null, 'auto_call_conditional', '16', '条件表达式', '\/pages\/autotrack\/autotrack\?case=conditional', null, null, conditionEnabled, 1\) \? onConditionalTrue\(\) : onConditionalFalse\(\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /@change="_gioAutoTrackDispatch\(\$event, 0, \$event, null, null, null, null, null, null, conditionEnabled, 1\) \? onChangeTrue\(\$event\) : onChangeFalse\(\$event\)"/)
...

{
  ...
  const output = transform(code)
  assert.match(output, /gioHandleAutoConditionalChange\(\$event, conditionEnabled, 'onChangeTrue', 'onChangeFalse', null, null, null, null, null, null, null, \$event\) \? onChangeTrue\(\$event\) : onChangeFalse\(\$event\)/)
